/*
  ...
  Supports:
  - legacy single-solution questions
  - grouped questions using parts[]
  - approved diagram engines
*/
...
function renderWorkedSolution(targetId, data) {
  ...
  questions.forEach((question, index) => {
    ...
    if (Array.isArray(question.parts) && question.parts.length) {
      if (question.criteria?.length) {
        section.innerHTML += renderCriteriaTable(question.criteria);
      }

      question.parts.forEach((part, partIndex) => {
        const partBlock = document.createElement("div");
        partBlock.className = "part";

        partBlock.innerHTML = `
          <div class="part-label">${escapeHTML(part.partLabel || "")}</div>

          ${part.questionText ? `
            <div class="part-question-text">
              ${escapeHTML(part.questionText)}
            </div>
          ` : ""}

          ${renderCriteriaTable(part.criteria || [])}

          <div class="sample-answer-title">Sample answer:</div>

          <div class="solution-render-area" id="solution-area-${index}-${partIndex}"></div>

          ${renderListBlock("Common errors", part.commonErrors)}
          ${renderListBlock("Teacher notes", part.teacherNotes)}
        `;

        section.appendChild(partBlock);
      });
    } else {
      section.innerHTML += `
  ...
  questions.forEach((question, index) => {
    if (Array.isArray(question.parts) && question.parts.length) {
      question.parts.forEach((part, partIndex) => {
        renderQuestionSolution(`solution-area-${index}-${partIndex}`, part.solution || {});
      });
    } else {
  ...
  switch (diagram.engine) {
    ...
    case "network-graph":
      if (typeof renderNetworkGraph === "function") {
        renderNetworkGraph(id, config);
      }
      break;

    case "box-plot":
      if (typeof renderBoxPlot === "function") {
        renderBoxPlot(id, config);
      }
      break;
  ...
  return `
    <div class="solution-box">
      ...
      ${solution.finalAnswer ? `
        <div class="final-answer">${solution.finalAnswer}</div>
      ` : ""}
    </div>
  `;
}
